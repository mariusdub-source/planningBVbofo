                    ...
                    {visibleEmployeeIndexes.map((employeeIndex) => {
                      ...
                      return (
                        <tr key={employeeIndex}>
                          ...
                          <td style={{ backgroundColor: placeColor(shift.counter) }}>{renderPlaceSelect(employeeIndex, selectedDayKey, shift)}</td>
                ...
                {visibleEmployeeIndexes.map((employeeIndex) => (
                  <React.Fragment key={employeeIndex}>
                    ...
                    {days.map((day) => {
                      const shift = getShift(data, weekKey, employeeIndex, day.key);
                      return (
                        <div className="shift-card" key={`${employeeIndex}-${day.key}`} style={{ backgroundColor: placeColor(shift.counter) }}>
                    ...
                    <tbody>
                      {employees.map((employee, employeeIndex) => {
                        ...
                        return (
                          <tr key={employeeIndex}>
                            <th>{employee}</th>
                            <td className="global-place-cell" style={{ backgroundColor: placeColor(shift.counter) }}>{shift.counter || '—'}</td>
                            <td>{shift.start || '—'}</td>
                            <td>{shift.end || '—'}</td>
                            <td>{shift.breakMinutes ? `${shift.breakMinutes} h` : '—'}</td>
                            <td className="total-cell">{formatHours(shiftMinutes(shift))}</td>
                            <td>{shift.note || '—'}</td>
                          </tr>
                        );
