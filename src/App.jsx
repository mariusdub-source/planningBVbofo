...
const STORAGE_KEY = 'planning-maritime-affichage-simple-detail-v1';
...
export default function App() {
  ...
  function exportCsv() {
    ...
    link.href = url;
    link.download = 'planning-maritime.csv';
    link.click();
  ...
  const selected = selectedCell ? planning[selectedCell.employee][selectedCell.day] : null;
  const selectedTotal = selected ? calculateDayTotal(selected) : 0;
    ...
    <main className="app">
      <header className="topbar">
        <div className="title-zone">
          ...
          <div>
            <h1>Planning Maritime</h1>
            <p>{adminMode ? 'Mode admin actif : modification autorisée.' : 'Lecture seule : cliquez sur une case pour voir le détail.'}</p>
          </div>
        </div>
      ...
      {page === 'planning' && (
        <>
          <section className="toolbar">
            ...
            <span>🗓️ affichage principal simplifié</span>
            <span>👆 cliquez sur une case pour le détail</span>
                ...
                <React.Fragment key={employee}>
                  ...
                  {DAYS.map((day) => {
                    ...
                    return (
                      <button
                        type="button"
                        className={`summary-card ${cell.guichet || cell.poste ? 'filled' : ''}`}
                        key={`${employee}-${day}`}
          ...
          <div className="table-wrapper totals-wrapper">
            <table className="totals-table">
              ...
              <tbody>{weeklyTotals.map((row) => (
                <tr key={row.employee}><th>{row.employee}</th><td>{row.workedDays}</td><td>{minutesToHours(row.totalMinutes)}</td><td>{formatPosteTotals(row.posteTotals)}</td></tr>
              ))}</tbody>
            </table>
          </div>
        </section>
        ...
        <div className="modal-backdrop" onClick={() => setSelectedCell(null)}>
          <section className="detail-modal" onClick={(event) => event.stopPropagation()}>
            <header className="modal-header">
              <div>
                <h2>{selectedCell.employee}</h2>
                <p>{selectedCell.day}</p>
              </div>
              <button type="button" onClick={() => setSelectedCell(null)}>Fermer</button>
            </header>

            <div className="detail-grid">
              <label>
                <span>Guichet</span>
                <select value={selected.guichet} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'guichet', event.target.value)} disabled={!adminMode}>
              ...
              <label>
                <span>Poste</span>
                <select value={selected.poste} onChange={(event) => applyPoste(selectedCell.employee, selectedCell.day, event.target.value)} disabled={!adminMode}>
              ...
              <label>
                <span>Début</span>
                <input type="time" value={selected.start} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'start', event.target.value)} disabled={!adminMode} />
              </label>

              <label>
                <span>Fin</span>
                <input type="time" value={selected.end} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'end', event.target.value)} disabled={!adminMode} />
              </label>

              <label>
                <span>Pause de</span>
                <input type="time" value={selected.pauseStart} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'pauseStart', event.target.value)} disabled={!adminMode} />
              </label>

              <label>
                <span>Pause à</span>
                <input type="time" value={selected.pauseEnd} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'pauseEnd', event.target.value)} disabled={!adminMode} />
              </label>

              <label>
                <span>Total manuel</span>
                <input type="text" value={selected.manualTotal} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'manualTotal', event.target.value)} disabled={!adminMode} placeholder="ex: 7,50" />
              </label>

              <label className="wide">
                <span>Note</span>
                <textarea value={selected.note} onChange={(event) => updateCell(selectedCell.employee, selectedCell.day, 'note', event.target.value)} disabled={!adminMode} placeholder="Saisie libre" />
              </label>
            </div>

            <div className="modal-total">
              <span>Total journée</span>
              <strong>{minutesToHours(selectedTotal)}</strong>
            </div>
          </section>
        </div>
      )}
