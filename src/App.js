                            ...
                            {segmentDays.map((item) => {
                              ...
                              return (
                                <div className={`shift-card ${row.source === 'actual' ? 'actual-row' : 'theoretical-row'} ${!canEditRow ? 'readonly-shift' : ''}`} key={`${targetWeekKey}-${employeeIndex}-${row.source}-${item.dayKey}`} style={{ backgroundColor: placeColor(shift.counter) }}>
                                  {canEditRow ? (
                                    <>
                                      <label>Guichet{renderPlaceSelect(employeeIndex, item.dayKey, shift, updater)}</label>
                                      <label>Shift{renderShiftPresetSelect(employeeIndex, item.dayKey, shift, row.source, targetWeekKey)}</label>
                                  ...
                                  {canEditRow ? (
                                    <label style={{ marginTop: 8 }}>
                                      ...
                                      <input
                                        type="text"
                                        value={shift.note || ''}
                                        onChange={(e) => updater(employeeIndex, item.dayKey, 'note', e.target.value)}
          ...
          {activeView === 'month' && (
            <section className="card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '18px 20px', borderBottom: '1px solid #e5e7eb', flexWrap: 'wrap' }}>
                <div>
                  <h2 style={{ margin: 0 }}>Aperçu mensuel — {new Intl.DateTimeFormat('fr-FR', { month: 'long', year: 'numeric' }).format(selectedDateObject)}</h2>
                  <p style={{ margin: '5px 0 0', color: '#64748b' }}>Vue mensuelle au format Planning visuel, avec une case colorée par jour.</p>
                </div>
                <span className="badge">Vue mensuelle</span>
              </div>

              <div style={{ overflowX: 'auto', background: '#f8fafc' }}>
                {(() => {
                  const monthDays = getMonthDays(selectedDateObject);
                  const minGridWidth = 150 + (monthDays.length * 135) + 95;
                  return (
                    <div style={{ minWidth: minGridWidth }}>
                      <div style={{ display: 'grid', gridTemplateColumns: `150px repeat(${monthDays.length}, minmax(135px, 1fr)) 95px`, position: 'sticky', top: 0, zIndex: 4, background: '#fff', borderBottom: '1px solid #dbe3ec' }}>
                        ...
                        {monthDays.map((calendarDate) => {
                          ...
                          const weekday = new Intl.DateTimeFormat('fr-FR', { weekday: 'short' }).format(calendarDate).replace('.', '');
                          return (
                            <div key={toDateInputValue(calendarDate)} style={{ textAlign: 'center', padding: '9px 6px', borderLeft: '1px solid #eef2f7' }}>
                              <div style={{ fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>{weekday.slice(0, 3)}</div>
                              <div style={{ width: 30, height: 30, margin: '4px auto 0', borderRadius: '50%', display: 'grid', placeItems: 'center', fontWeight: 800, background: isToday ? '#2563eb' : 'transparent', color: isToday ? '#fff' : '#0f172a' }}>{calendarDate.getDate()}</div>
                            </div>
                          );
                        })}
                      ...
                      {visibleEmployeeIndexes.map((employeeIndex) => (
                        <div key={employeeIndex} style={{ display: 'grid', gridTemplateColumns: `150px repeat(${monthDays.length}, minmax(135px, 1fr)) 95px`, minHeight: 82, background: '#fff', borderBottom: '1px solid #e8edf3' }}>
                          <div style={{ padding: '12px 10px', display: 'flex', alignItems: 'center', gap: 9, fontWeight: 750, position: 'sticky', left: 0, zIndex: 2, background: '#fff', borderRight: '1px solid #eef2f7' }}>
                            <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#e2e8f0', color: '#334155', display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 800, flex: '0 0 auto' }}>{String(employees[employeeIndex] || '?').slice(0, 2).toUpperCase()}</div>
                            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{employees[employeeIndex]}</span>
                          </div>
                          {monthDays.map((calendarDate) => {
                            const targetWeekKey = weekKeyFromDate(calendarDate);
                            const targetDayKey = dayKeyFromDate(calendarDate);
                            ...
                            return (
                              <div key={toDateInputValue(calendarDate)} style={{ padding: 7, borderLeft: '1px solid #eef2f7', minWidth: 0 }}>
                                {hasShift ? (
                                  <div
                                    title={`${shift.counter || ''} ${shift.start || ''}-${shift.end || ''}`}
                                    style={{ width: '100%', minHeight: 64, border: 'none', borderRadius: 7, padding: '8px 9px', textAlign: 'left', background: placeColor(shift.counter), boxShadow: 'inset 0 0 0 1px rgba(15,23,42,.08)', color: '#0f172a', boxSizing: 'border-box' }}
                                  >
                                    ...
                                    {shift.note && shift.note !== shift.counter ? (
                                      <div style={{ fontSize: 10, marginTop: 4, fontStyle: 'italic', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{shift.note}</div>
                                    ) : null}
                                  </div>
                                ) : isOff ? (
                          ...
                          <div style={{ borderLeft: '1px solid #eef2f7', display: 'grid', placeItems: 'center', fontWeight: 850, fontSize: 13 }}>{formatHours(monthTotalForEmployee(employeeIndex))}</div>
                        </div>
                      ))}
                    </div>
                  );
