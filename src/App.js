...
function App() {
  ...
  const updateActualShift = (employeeIndex, dayKey, field, value) => {
    if (!isEmployeeUnlocked || Number(auth.employeeIndex) !== Number(employeeIndex)) return;
    const current = getActualShift(weekKey, employeeIndex, dayKey);
    const next = {
      ...actualData,
      [weekKey]: {
        ...(actualData[weekKey] || {}),
        [employeeIndex]: {
          ...((actualData[weekKey] || {})[employeeIndex] || {}),
          [dayKey]: { ...current, [field]: value }
        }
      }
    };
    ...
    persistValidations({
      ...validations,
      [weekKey]: { ...(validations[weekKey] || {}), [employeeIndex]: false }
    });
  };

  const setWeekValidated = (employeeIndex, checked) => {
    if (!isEmployeeUnlocked || Number(auth.employeeIndex) !== Number(employeeIndex)) return;
    persistValidations({
      ...validations,
      [weekKey]: { ...(validations[weekKey] || {}), [employeeIndex]: checked }
    });
  };
    ...
    return (
      <div className="time-selectors">
        <select
          aria-label="Heure"
          value={hour}
          onChange={(e) => updater(employeeIndex, dayKey, field, buildTime(e.target.value, selectedMinuteDecimal))}
        >
        ...
        <select
          ...
          onChange={(e) => updater(employeeIndex, dayKey, field, buildTime(hour || '00', e.target.value))}
        >
    ...
    <div className="app-shell">
      <header className="hero">
        <div>
          ...
          <p className="subtitle">L’administrateur prépare le planning théorique. Chaque salarié saisit ensuite ses heures réelles et valide sa semaine.</p>
        </div>
            ...
            <section className="card">
              ...
              <div className="table-wrap">
                <table className="compact-table">
                  <thead>
                    <tr><th>Salarié</th><th>Guichet</th><th>Début</th><th>Fin</th><th>Pause</th><th>Total</th><th>Note</th></tr>
                  </thead>
                  <tbody>
                    {visibleEmployeeIndexes.map((employeeIndex) => {
                      ...
                      return (
                        <tr key={employeeIndex}>
                          ...
                          <td style={{ backgroundColor: placeColor(shift.counter) }}>{isAdmin ? renderPlaceSelect(employeeIndex, selectedDayKey, shift) : (shift.counter || '—')}</td>
                          ...
                          <td>{isAdmin ? renderPauseInput(employeeIndex, selectedDayKey, shift) : `${shift.breakMinutes || '0'} h`}</td>
            ...
            <section className="card">
              <div className="section-title">
                <div>
                  ...
                  <p>{isAdmin ? 'Comparaison entre le planning théorique et les heures validées par chaque salarié.' : 'Saisissez vos heures réellement effectuées, puis validez votre semaine.'}</p>
                </div>
                {!isAdmin && visibleEmployeeIndexes.length === 1 && (
                  <label className={`week-validation ${isWeekValidated(visibleEmployeeIndexes[0]) ? 'validated' : ''}`}>
                    <input
                      type="checkbox"
                      checked={isWeekValidated(visibleEmployeeIndexes[0])}
                      onChange={(e) => setWeekValidated(visibleEmployeeIndexes[0], e.target.checked)}
                    />
                    Semaine validée
                  </label>
                )}
              ...
              <div className="planning-grid admin-comparison-grid">
                <div className="sticky-left header-cell">Salarié / type</div>
                {days.map((day, index) => (
                ...
                {visibleEmployeeIndexes.map((employeeIndex) => {
                  const rows = isAdmin
                    ? [
                        ...
                        { type: isWeekValidated(employeeIndex) ? 'Compté ✓' : 'Compté — en attente', source: 'actual', editable: false }
                      ]
                    : [{ type: 'Heures réelles', source: 'actual', editable: true }];
                  return rows.map((row) => (
                    <React.Fragment key={`${employeeIndex}-${row.source}`}>
                      ...
                      {days.map((day) => {
                        const shift = row.source === 'actual'
                          ? getActualShift(weekKey, employeeIndex, day.key)
                          : getShift(data, weekKey, employeeIndex, day.key);
                        const canEditRow = row.editable && (isAdmin || isEmployeeUnlocked);
                        return (
                          <div className={`shift-card ${row.source === 'actual' ? 'actual-row' : 'theoretical-row'} ${!canEditRow ? 'readonly-shift' : ''}`} key={`${employeeIndex}-${row.source}-${day.key}`} style={{ backgroundColor: placeColor(shift.counter) }}>
                            {canEditRow ? (
                              <>
                                <label>Guichet{renderPlaceSelect(employeeIndex, day.key, shift, row.source === 'actual' ? updateActualShift : updateShift)}</label>
                                <label>Début{renderTimeSelect(employeeIndex, day.key, 'start', shift.start, row.source === 'actual' ? updateActualShift : updateShift)}</label>
                                <label>Fin{renderTimeSelect(employeeIndex, day.key, 'end', shift.end, row.source === 'actual' ? updateActualShift : updateShift)}</label>
                                <label>Pause{renderPauseInput(employeeIndex, day.key, shift, row.source === 'actual' ? updateActualShift : updateShift)}</label>
                              </>
                            ) : (
                              <div className="readonly-shift-content">
                                ...
                                {!isWeekValidated(employeeIndex) && row.source === 'actual' && <em>Non validé</em>}
                              </div>
                            )}
                      ...
                      <div className={`week-total-cell ${row.source === 'actual' ? 'actual-row' : ''}`}>
                        {row.source === 'actual' ? formatHours(actualWeekTotalForEmployee(employeeIndex)) : formatHours(weekTotalForEmployee(employeeIndex))}
                      </div>
                    </React.Fragment>
                  ));
                })}
                    ...
                    <tbody>
                      {employees.map((employee, employeeIndex) => {
                        ...
                        return (
                          <tr key={employeeIndex}>
                            <th>{employee}</th>
                            <td className="global-place-cell" style={{ backgroundColor: placeColor(shift.counter) }}>{isAdmin ? renderPlaceSelect(employeeIndex, selectedDayKey, shift) : (shift.counter || '—')}</td>
                            <td>{isAdmin ? renderTimeSelect(employeeIndex, selectedDayKey, 'start', shift.start) : (shift.start || '—')}</td>
                            <td>{isAdmin ? renderTimeSelect(employeeIndex, selectedDayKey, 'end', shift.end) : (shift.end || '—')}</td>
