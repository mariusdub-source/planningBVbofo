...
const defaultShiftPresets = [
  ...
  { name: 'Bureau 07h00', place: 'Bureau', start: '07:00', end: '15:30', breakMinutes: '0,50', breakStart: '12:00', breakEnd: '12:30' },
  { name: 'Bureau 07h30', place: 'Bureau', start: '07:30', end: '16:00', breakMinutes: '0,50', breakStart: '12:00', breakEnd: '12:30' },
  { name: 'Bureau 08h30', place: 'Bureau', start: '08:30', end: '17:00', breakMinutes: '0,50', breakStart: '13:00', breakEnd: '13:30' },
  ...
  { name: 'Max journée 08h30', place: 'Sainte-Maxime', start: '08:30', end: '17:00', breakMinutes: '1,50', breakStart: '13:00', breakEnd: '14:30' },
  { name: 'Max journée 09h00', place: 'Sainte-Maxime', start: '09:00', end: '18:00', breakMinutes: '1,50', breakStart: '13:00', breakEnd: '14:30' },
...
const pdfPauseTimesByShift = {
  'aqua': { breakStart: '12:00', breakEnd: '13:30', breakMinutes: '1,50' },
  'bureau 07h00': { breakStart: '12:00', breakEnd: '12:30', breakMinutes: '0,50' },
  'bureau 07h30': { breakStart: '12:00', breakEnd: '12:30', breakMinutes: '0,50' },
  'bureau 08h30': { breakStart: '13:00', breakEnd: '13:30', breakMinutes: '0,50' },
  'issambres journée': { breakStart: '12:30', breakEnd: '13:45', breakMinutes: '1,25' },
  'max après-midi': { breakStart: '17:00', breakEnd: '18:00', breakMinutes: '1' },
  'max fermeture': { breakStart: '15:00', breakEnd: '16:00', breakMinutes: '1' },
  'max fermeture tardive': { breakStart: '15:00', breakEnd: '16:15', breakMinutes: '1,25' },
  'max journée 08h30': { breakStart: '13:00', breakEnd: '14:30', breakMinutes: '1,50' },
  'max journée 09h00': { breakStart: '13:00', breakEnd: '14:30', breakMinutes: '1,50' },
  'max ouverture 07h00': { breakStart: '12:00', breakEnd: '12:30', breakMinutes: '0,50' },
  'max ouverture 07h30': { breakStart: '12:00', breakEnd: '12:30', breakMinutes: '0,50' },
  'max renfort 08h00': { breakStart: '12:00', breakEnd: '13:00', breakMinutes: '1' },
  'max renfort 09h30': { breakStart: '13:00', breakEnd: '14:30', breakMinutes: '1,50' },
  'pg capit journée': { breakStart: '12:45', breakEnd: '13:30', breakMinutes: '0,75' },
  'pg capit marche': { breakStart: '12:45', breakEnd: '13:30', breakMinutes: '0,75' },
  'pg église journée': { breakStart: '12:35', breakEnd: '13:20', breakMinutes: '0,75' },
  'trop journée': { breakStart: '13:30', breakEnd: '13:50', breakMinutes: '0,33' },
  'trop matin': { breakStart: '13:10', breakEnd: '13:15', breakMinutes: '0,08' },
  'trop soir': { breakStart: '20:00', breakEnd: '20:20', breakMinutes: '0,33' }
};

function withPdfPauseTimes(item, label = '') {
  if (!item || item.breakStart || item.breakEnd) return item;
  const normalizedLabel = String(label || item.name || item.note || '').toLocaleLowerCase('fr-FR');
  const presetPause = pdfPauseTimesByShift[normalizedLabel];
  if (presetPause) return { ...item, ...presetPause };
  const pauseMinutes = pauseDecimalToMinutes(item.breakMinutes || 0);
  if (normalizedLabel.includes('max journée') && pauseMinutes === 90) return { ...item, breakStart: '13:00', breakEnd: '14:30' };
  if (normalizedLabel.includes('bureau') && pauseMinutes === 30) return { ...item, breakStart: '13:00', breakEnd: '13:30' };
  return item;
}

function migrateShiftPresetPauses(storedPresets) {
  const presets = Array.isArray(storedPresets) && storedPresets.length ? storedPresets : defaultShiftPresets;
  return presets.map((preset) => withPdfPauseTimes(preset, preset?.name));
}

function emptyShift() {
  return { start: '', end: '', breakMinutes: '0', breakStart: '', breakEnd: '', counter: '', note: '' };
}
...
function formatPauseDetails(shift) {
  const duration = String(shift?.breakMinutes || '0');
  const start = formatClock(shift?.breakStart) || '..h..';
  const end = formatClock(shift?.breakEnd) || '..h..';
  return `Pause : ${duration} h — De ${start} à ${end}`;
}
...
function getShift(data, weekKey, employeeIndex, dayKey, depth = 0) {
  ...
  if (explicitShift) return withPdfPauseTimes(explicitShift);
...
function App() {
  ...
  const [shiftPresets, setShiftPresets] = useState(() => migrateShiftPresetPauses(readJSON(SHIFT_PRESETS_KEY, defaultShiftPresets)));
  ...
  const [pointageSaved, setPointageSaved] = useState(false);
  ...
  const monthWeekSegments = useMemo(() => getMonthWeekSegments(selectedDateObject), [selectedDateObject]);
  const selectedMonthWeekSegmentIndex = useMemo(() => {
    const selectedDay = toDateInputValue(selectedDateObject);
    const index = monthWeekSegments.findIndex((segment) => segment.some((item) => toDateInputValue(item.date) === selectedDay));
    return index >= 0 ? index : 0;
  }, [monthWeekSegments, selectedDateObject]);
  const visibleHourSheetSegments = isAdmin
    ? monthWeekSegments.slice(selectedMonthWeekSegmentIndex, selectedMonthWeekSegmentIndex + 1)
    : monthWeekSegments;
  ...
  const updateShiftPreset = (index, field, value) => {
    const previousPreset = shiftPresets[index];
    const updatedPreset = { ...previousPreset, [field]: value };
    const next = shiftPresets.map((preset, presetIndex) => presetIndex === index ? updatedPreset : preset);
    persistShiftPresets(next);

    if (['breakMinutes', 'breakStart', 'breakEnd'].includes(field)) {
      const presetName = String(previousPreset?.name || '').trim().toLocaleLowerCase('fr-FR');
      const applyPauseToPresetEntries = (source) => {
        let changed = false;
        const updatedSource = Object.fromEntries(Object.entries(source || {}).map(([storedWeekKey, employeesByIndex]) => [
          storedWeekKey,
          Object.fromEntries(Object.entries(employeesByIndex || {}).map(([employeeIndex, shiftsByDay]) => [
            employeeIndex,
            Object.fromEntries(Object.entries(shiftsByDay || {}).map(([dayKey, shift]) => {
              const usesPreset = String(shift?.note || '').trim().toLocaleLowerCase('fr-FR') === presetName;
              if (!usesPreset) return [dayKey, shift];
              changed = true;
              return [dayKey, {
                ...shift,
                breakMinutes: updatedPreset.breakMinutes || '0',
                breakStart: updatedPreset.breakStart || '',
                breakEnd: updatedPreset.breakEnd || ''
              }];
            }))
          ]))
        ]));
        return changed ? updatedSource : source;
      };

      const nextData = applyPauseToPresetEntries(data);
      const nextActualData = applyPauseToPresetEntries(actualData);
      if (nextData !== data) persistData(nextData);
      if (nextActualData !== actualData) persistActualData(nextActualData);
    }
  };

  const addShiftPreset = () => {
    persistShiftPresets([...shiftPresets, { name: 'Nouveau shift', place: places[0] || '', start: '08:00', end: '16:00', breakMinutes: '0', breakStart: '', breakEnd: '' }]);
  };
  ...
  const updatePointageField = (field, value) => {
    if (!isEmployeeUnlocked) return;
    const employeeIndex = Number(auth.employeeIndex || 0);
    const targetWeekKey = weekKeyFromDate(selectedDateObject);
    const targetDayKey = dayKeyFromDate(selectedDateObject);
    const current = getActualShift(targetWeekKey, employeeIndex, targetDayKey);
    const nextShift = { ...current, [field]: value };
    if (field === 'breakStart' || field === 'breakEnd') {
      const pauseStart = timeToMinutes(nextShift.breakStart);
      const pauseEnd = timeToMinutes(nextShift.breakEnd);
      if (pauseStart !== null && pauseEnd !== null) {
        let pauseDuration = pauseEnd - pauseStart;
        if (pauseDuration < 0) pauseDuration += 24 * 60;
        nextShift.breakMinutes = String(Math.round((pauseDuration / 60) * 100) / 100).replace('.', ',');
      }
    }
    persistActualData({
      ...actualData,
    ...
    setPointageSaved(false);
  };

  const savePointageDay = () => {
    if (!isEmployeeUnlocked) return;
    const employeeIndex = Number(auth.employeeIndex || 0);
    const targetWeekKey = weekKeyFromDate(selectedDateObject);
    const targetDayKey = dayKeyFromDate(selectedDateObject);
    ...
    const savedShift = {
      ...current,
      pointageRecorded: true,
      pointageSavedAt: new Date().toISOString(),
      pointageSource: 'declaratif'
    };
    persistActualData({
      ...actualData,
      [targetWeekKey]: {
    ...
    setPointageSaved(true);
  };

  const applyTheoreticalPreset = (employeeIndex, dayKey, preset, targetWeekKey = weekKey) => {
    if (!isAdmin || !preset) return;
    const current = getShift(data, targetWeekKey, employeeIndex, dayKey);
    const nextShift = { ...current, start: preset.start, end: preset.end, breakMinutes: preset.breakMinutes, breakStart: preset.breakStart || '', breakEnd: preset.breakEnd || '', counter: preset.place, note: preset.name };
  ...
  const setWeekValidated = (employeeIndex, checked, targetWeekKey = weekKey) => {
    const employeeCanValidate = isEmployeeUnlocked && Number(auth.employeeIndex) === Number(employeeIndex);
    if (!isAdmin && !employeeCanValidate) return;
    persistValidations({
      ...validations,
      [targetWeekKey]: {
        ...(validations[targetWeekKey] || {}),
        [employeeIndex]: checked ? { validated: true, validatedAt: new Date().toISOString() } : false
      }
    });
  };

  const isWeekValidated = (employeeIndex, targetWeekKey = weekKey) => {
    const validation = validations?.[targetWeekKey]?.[employeeIndex];
    return validation === true || Boolean(validation?.validated);
  };

  const weekValidationDate = (employeeIndex, targetWeekKey = weekKey) => {
    const validation = validations?.[targetWeekKey]?.[employeeIndex];
    if (!validation || typeof validation !== 'object' || !validation.validatedAt) return '';
    const date = new Date(validation.validatedAt);
  ...
  const monthTotalForEmployee = (employeeIndex, source = 'theoretical') => {
    ...
    for (let dayNumber = 1; dayNumber <= lastDay; dayNumber += 1) {
      ...
      const shift = source === 'actual'
        ? getActualShift(targetWeekKey, employeeIndex, targetDayKey)
        : getShift(data, targetWeekKey, employeeIndex, targetDayKey);
  ...
  const renderTimeSelect = (employeeIndex, dayKey, field, value, updater = updateShift) => (
    <div className="time-manual-wrap">
      <input
        ...
        placeholder="07:30"
        aria-label={{ start: 'Heure de début', end: 'Heure de fin', breakStart: 'Début de pause', breakEnd: 'Fin de pause' }[field] || 'Horaire'}
      />
    </div>
  );
  ...
  const renderPauseInput = (employeeIndex, dayKey, shift, updater = updateShift) => {
    ...
    return (
      <div className="pause-input-wrap">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto 1fr', gap: 5, alignItems: 'center', marginTop: 6 }}>
          <small>De</small>
          {renderTimeSelect(employeeIndex, dayKey, 'breakStart', shift.breakStart, updater)}
          <small>à</small>
          {renderTimeSelect(employeeIndex, dayKey, 'breakEnd', shift.breakEnd, updater)}
        </div>
      </div>
    );
  };
  ...
  const scrollMobileTable = (elementId, direction) => {
    const element = typeof document !== 'undefined' ? document.getElementById(elementId) : null;
    if (element) element.scrollBy({ left: direction * Math.min(260, Math.max(185, element.clientWidth * 0.72)), behavior: 'smooth' });
  };

  if (!appUnlocked) {
    return (
      <div
        className="login-screen access-login-screen"
        style={{ background: '#f8fafc', backgroundImage: 'none' }}
      >
  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .month-weeks-list { overflow: visible !important; }
        .admin-week-sticky { position: sticky; top: 8px; z-index: 100; margin-bottom: 12px; padding: 14px 16px; border-bottom: 1px solid #dbe3ec; background: rgba(255,255,255,.98); box-shadow: 0 6px 16px rgba(15,23,42,.12); backdrop-filter: blur(8px); }
        .pointage-shell { max-width: 560px; margin: 0 auto; padding: 0; overflow: hidden; }
        .pointage-header { padding: 22px 24px; background: #0f857e; color: #fff; }
        .pointage-header-main { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
        .pointage-header h2 { margin: 0; color: #fff; }
        .pointage-avatar { width: 44px; height: 44px; flex: 0 0 auto; border-radius: 50%; display: grid; place-items: center; background: rgba(255,255,255,.2); color: #fff; font-weight: 800; }
        .pointage-day-nav { display: grid; grid-template-columns: 42px 1fr 42px; align-items: center; gap: 8px; margin-top: 10px; }
        .pointage-day-nav button { min-width: 42px; min-height: 40px; padding: 6px; border: 1px solid rgba(255,255,255,.45); background: rgba(255,255,255,.12); color: #fff; }
        .pointage-date { text-align: center; font-weight: 750; text-transform: capitalize; }
        .pointage-body { display: grid; gap: 16px; padding: 24px; }
        .pointage-intro { margin: 0; color: #64748b; }
        .pointage-two-columns { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }
        .pointage-pause { padding: 16px; border-radius: 12px; background: #e2f7f3; }
        .pointage-pause-title { display: block; margin-bottom: 12px; color: #065f5b; }
        .pointage-total { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 14px 0; border-top: 1px solid #dbe3ec; border-bottom: 1px solid #dbe3ec; }
        .pointage-total strong:last-child { font-size: 26px; color: #0f766e; }
        .pointage-save { width: 100%; min-height: 50px; }
        .pointage-success { padding: 12px; border-radius: 8px; background: #e2f7f3; color: #065f5b; font-weight: 700; text-align: center; }
        .pointage-month { margin-top: 8px; padding-top: 20px; border-top: 1px solid #dbe3ec; }
        .pointage-month-header { display: grid; grid-template-columns: 40px 1fr 40px; align-items: center; gap: 8px; margin-bottom: 12px; }
        .pointage-month-header h3 { margin: 0; text-align: center; text-transform: capitalize; }
        .pointage-month-header button { min-width: 40px; min-height: 40px; padding: 6px; }
        .pointage-calendar-weekdays, .pointage-calendar { display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap: 5px; }
        .pointage-calendar-weekdays span { padding: 4px 0; color: #64748b; font-size: 10px; font-weight: 800; text-align: center; text-transform: uppercase; }
        .pointage-calendar-day, .pointage-calendar-empty { min-width: 0; min-height: 54px; }
        .pointage-calendar-day { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; padding: 5px 2px; border: 1px solid #dbe3ec; border-radius: 8px; background: #fff; color: #0f172a; }
        .pointage-calendar-day small { font-size: 9px; color: #64748b; white-space: nowrap; }
        .pointage-calendar-day.pointed { background: #e2f7f3; border-color: #72d6ca; color: #065f5b; }
        .pointage-calendar-day.selected { box-shadow: inset 0 0 0 2px #0f857e; }
        .pointage-month-total { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-top: 12px; padding: 12px 0; }
        .pointage-month-total strong:last-child { color: #0f766e; font-size: 21px; }
        @media (max-width: 768px) {
          .app-shell { width: 100%; max-width: 100vw; overflow-x: hidden; }
          .card { max-width: 100%; box-sizing: border-box; }
          .mobile-horizontal-scroll { display: block; padding-bottom: 8px; }
          .mobile-horizontal-scroll .admin-comparison-grid { min-width: max-content !important; }
          .global-view-table { min-width: 900px; }
          ...
          .mobile-scroll-controls button { flex: 0 0 auto; padding: 8px 11px; }
          .mobile-scroll-controls span { color: #64748b; font-size: 11px; text-align: center; }
          .toolbar-actions { max-width: 100%; }
          .toolbar-actions .week-arrow-button span { display: none; }
          .pointage-shell { width: 100%; }
          .pointage-header, .pointage-body { padding: 18px; }
        }
        @media (max-width: 390px) { .pointage-two-columns { grid-template-columns: 1fr; } }
      `}</style>
      ...
      {!canUsePlanning ? (
        ...
        <>
          ...
          <nav className="tabs">
            {!isAdmin && isEmployeeUnlocked && <button className={activeView === 'pointage' ? 'active' : ''} onClick={() => setActiveView('pointage')}>Pointage</button>}
          ...
          {activeView === 'pointage' && !isAdmin && isEmployeeUnlocked && (() => {
            const employeeIndex = Number(auth.employeeIndex || 0);
            const targetWeekKey = weekKeyFromDate(selectedDateObject);
            const targetDayKey = dayKeyFromDate(selectedDateObject);
            const shift = getActualShift(targetWeekKey, employeeIndex, targetDayKey);
            const pointageMonthDays = getMonthDays(selectedDateObject);
            const pointageMonthOffset = (pointageMonthDays[0].getDay() + 6) % 7;
            const pointageMonthTotal = pointageMonthDays.reduce((total, calendarDate) => {
              const storedShift = actualData?.[weekKeyFromDate(calendarDate)]?.[employeeIndex]?.[dayKeyFromDate(calendarDate)];
              return total + (storedShift ? shiftMinutes(storedShift) : 0);
            }, 0);
            return (
              <section className="card pointage-shell">
                <div className="pointage-header">
                  <div className="pointage-header-main">
                    <div><h2>Mon pointage</h2><small>{employees[employeeIndex]} — horaires déclarés</small></div>
                    <div className="pointage-avatar">{String(employees[employeeIndex] || '?').slice(0, 2).toUpperCase()}</div>
                  </div>
                  <div className="pointage-day-nav">
                    <button type="button" onClick={() => { setSelectedDate(toDateInputValue(addDays(selectedDateObject, -1))); setPointageSaved(false); }} aria-label="Jour précédent" title="Jour précédent">←</button>
                    <div className="pointage-date">{displayLongDate(selectedDateObject)}</div>
                    <button type="button" onClick={() => { setSelectedDate(toDateInputValue(addDays(selectedDateObject, 1))); setPointageSaved(false); }} aria-label="Jour suivant" title="Jour suivant">→</button>
                  </div>
                </div>

                <div className="pointage-body">
                  <p className="pointage-intro">Je renseigne les horaires réellement effectués aujourd’hui.</p>
                  <label>Lieu de travail
                    <select value={shift.counter || ''} onChange={(e) => updatePointageField('counter', e.target.value)} style={{ backgroundColor: placeColor(shift.counter) }}>
                      <option value="">Sélectionner</option>
                      {places.filter(Boolean).map((place) => <option key={place} value={place}>{place}</option>)}
                    </select>
                  </label>

                  <div className="pointage-two-columns">
                    <label>Prise de poste
                      <input type="time" value={shift.start || ''} onChange={(e) => updatePointageField('start', e.target.value)} />
                    </label>
                    <label>Fin de poste
                      <input type="time" value={shift.end || ''} onChange={(e) => updatePointageField('end', e.target.value)} />
                    </label>
                  </div>

                  <div className="pointage-pause">
                    <strong className="pointage-pause-title">☕ Pause déclarée</strong>
                    <div className="pointage-two-columns">
                      <label>Début de pause
                        <input type="time" value={shift.breakStart || ''} onChange={(e) => updatePointageField('breakStart', e.target.value)} />
                      </label>
                      <label>Fin de pause
                        <input type="time" value={shift.breakEnd || ''} onChange={(e) => updatePointageField('breakEnd', e.target.value)} />
                      </label>
                    </div>
                    <small style={{ display: 'block', marginTop: 8, color: '#047857' }}>Durée calculée : {shift.breakMinutes || '0'} h</small>
                  </div>

                  <label>Observation facultative
                    <textarea rows="3" value={shift.note || ''} onChange={(e) => updatePointageField('note', e.target.value)} placeholder="Ajouter une précision si nécessaire" />
                  </label>

                  <div className="pointage-total">
                    <strong>Total déclaré</strong>
                    <strong style={{ fontSize: 26, color: '#0f766e' }}>{formatHours(shiftMinutes(shift))}</strong>
                  </div>

                  <button className="pointage-save" type="button" onClick={savePointageDay}>✓ Enregistrer ma journée</button>
                  {pointageSaved && <div className="pointage-success">Journée enregistrée dans les heures comptées.</div>}

                  <div className="pointage-month">
                    <div className="pointage-month-header">
                      <button className="secondary" type="button" onClick={() => { changeMonth(-1); setPointageSaved(false); }} aria-label="Mois précédent" title="Mois précédent">←</button>
                      <h3>Mes pointages — {new Intl.DateTimeFormat('fr-FR', { month: 'long', year: 'numeric' }).format(selectedDateObject)}</h3>
                      <button className="secondary" type="button" onClick={() => { changeMonth(1); setPointageSaved(false); }} aria-label="Mois suivant" title="Mois suivant">→</button>
                    </div>
                    <div className="pointage-calendar-weekdays">
                      {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map((label) => <span key={label}>{label}</span>)}
                    </div>
                    <div className="pointage-calendar">
                      {Array.from({ length: pointageMonthOffset }, (_, index) => <div className="pointage-calendar-empty" key={`empty-${index}`} />)}
                      {pointageMonthDays.map((calendarDate) => {
                        const dateValue = toDateInputValue(calendarDate);
                        const storedShift = actualData?.[weekKeyFromDate(calendarDate)]?.[employeeIndex]?.[dayKeyFromDate(calendarDate)];
                        const hasPointage = Boolean(storedShift && (storedShift.pointageRecorded || storedShift.start || storedShift.end || storedShift.breakStart || storedShift.breakEnd || storedShift.counter || storedShift.note));
                        const isSelected = dateValue === selectedDate;
                        return (
                          <button
                            type="button"
                            className={`pointage-calendar-day ${hasPointage ? 'pointed' : ''} ${isSelected ? 'selected' : ''}`}
                            key={dateValue}
                            onClick={() => { setSelectedDate(dateValue); setPointageSaved(false); }}
                            aria-label={`${displayLongDate(calendarDate)}${hasPointage ? `, ${formatHours(shiftMinutes(storedShift))} déclarées` : ', aucun pointage'}`}
                          >
                            <strong>{calendarDate.getDate()}</strong>
                            <small>{hasPointage ? formatHours(shiftMinutes(storedShift)) : '—'}</small>
                          </button>
                        );
                      })}
                    </div>
                    <div className="pointage-month-total"><strong>Total des pointages du mois</strong><strong>{formatHours(pointageMonthTotal)}</strong></div>
                  </div>
                </div>
              </section>
            );
          ...
          {activeView === 'week' && (
            <div className="month-weeks-list">
              {visibleHourSheetSegments.map((segmentDays) => {
                ...
                return (
                  <React.Fragment key={`${displayWeekKey}-${toDateInputValue(firstDate)}`}>
                    {isAdmin && (
                      <div className="card section-title admin-week-sticky">
                        <div>
                          <h2>Semaine du {displayDate(firstDate)} au {displayDate(lastDate)}</h2>
                          <p>Comparaison entre le planning théorique et les heures validées par chaque salarié.</p>
                        </div>
                        <div className="toolbar-actions">
                    ...
                    <section className="card month-week-card">
                    {!isAdmin && <div className="section-title">
                      <div>
                        <h2>Semaine du {displayDate(firstDate)} au {displayDate(lastDate)}</h2>
                        <p>Saisissez vos heures réellement effectuées, puis validez cette semaine.</p>
                      </div>
                      {visibleEmployeeIndexes.length === 1 && (
                    ...
                    <div className="mobile-scroll-controls">
                      <button type="button" className="secondary" onClick={() => scrollMobileTable(`hours-sheet-${displayWeekKey}`, -1)}>← Jour</button>
                      <span>Faites glisser le tableau horizontalement</span>
                      <button type="button" className="secondary" onClick={() => scrollMobileTable(`hours-sheet-${displayWeekKey}`, 1)}>Jour →</button>
                    </div>
                    <div id={`hours-sheet-${displayWeekKey}`} className="mobile-horizontal-scroll" tabIndex="0" aria-label="Feuille d'heures défilable horizontalement">
                      <div
                        className="planning-grid admin-comparison-grid"
                        style={{
                          gridTemplateColumns: isAdmin
                            ? `125px repeat(${segmentDays.length}, 185px) 110px 165px`
                            : `125px repeat(${segmentDays.length}, 185px) 110px`,
                          width: 'fit-content',
                          minWidth: '100%'
                        }}
                      >
                      ...
                      {isAdmin && <div className="header-cell total-week">Validation</div>}
                      ...
                      {visibleEmployeeIndexes.map((employeeIndex) => {
                        const rows = isAdmin
                          ? [
                              ...
                              {
                                type: isWeekValidated(employeeIndex, displayWeekKey) ? 'Compté ✓' : 'Compté — en attente',
                                source: 'actual',
                                editable: false,
                                validationDate: weekValidationDate(employeeIndex, displayWeekKey)
                              }
                            ]
                        ...
                        return rows.map((row) => (
                          <React.Fragment key={`${displayWeekKey}-${employeeIndex}-${row.source}`}>
                            ...
                            {isAdmin && (
                              <div className={`week-total-cell ${row.source === 'actual' ? 'actual-row' : ''}`} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 7, textAlign: 'center' }}>
                                {row.source === 'actual' ? (
                                  <>
                                    <label className={`week-validation ${isWeekValidated(employeeIndex, displayWeekKey) ? 'validated' : ''}`}>
                                      <input
                                        type="checkbox"
                                        checked={isWeekValidated(employeeIndex, displayWeekKey)}
                                        onChange={(e) => setWeekValidated(employeeIndex, e.target.checked, displayWeekKey)}
                                      />
                                      Valider le compté
                                    </label>
                                    {isWeekValidated(employeeIndex, displayWeekKey) && (
                                      <small>{row.validationDate ? `Validé le ${row.validationDate}` : 'Validation antérieure — date non disponible'}</small>
                                    )}
                                  </>
                                ) : <span>—</span>}
                              </div>
              ...
              <section className="card" style={{ marginTop: 18 }}>
                <div className="section-title">
                  <div>
                    <h2>Total du mois</h2>
                    <p>{new Intl.DateTimeFormat('fr-FR', { month: 'long', year: 'numeric' }).format(selectedDateObject)}</p>
                  </div>
                </div>
                {isAdmin ? (
                  <div className="mobile-horizontal-scroll" tabIndex="0">
                    <table className="compact-table" style={{ width: '100%' }}>
                      ...
                      <tbody>
                        {visibleEmployeeIndexes.map((employeeIndex) => (
                          <tr key={`month-total-${employeeIndex}`}>
                            <th>{employees[employeeIndex]}</th>
                            <td className="total-cell">{formatHours(monthTotalForEmployee(employeeIndex, 'theoretical'))}</td>
                            <td className="total-cell">{formatHours(monthTotalForEmployee(employeeIndex, 'actual'))}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '12px 4px 4px', flexWrap: 'wrap' }}>
                    <strong>{employees[visibleEmployeeIndexes[0]]}</strong>
                    <strong style={{ fontSize: 28, color: '#0f766e' }}>{formatHours(monthTotalForEmployee(visibleEmployeeIndexes[0], 'actual'))}</strong>
                  </div>
                )}
              </section>
            </div>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <div className="mobile-horizontal-scroll" style={{ background: '#f8fafc' }} tabIndex="0">
                                ...
                                {hasShift ? (
                                  <div style={{ width: '100%', minHeight: 64, borderRadius: 7, padding: '8px 9px', textAlign: 'left', background: placeColor(shift.counter), boxShadow: 'inset 0 0 0 1px rgba(15,23,42,.08)', color: '#0f172a', boxSizing: 'border-box' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 7 }}>
                                      <span style={{ minWidth: 0, fontSize: 12, fontWeight: 850, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{shift.start && shift.end ? `${shift.start} - ${shift.end}` : (shift.note || 'Shift')}</span>
                                      <strong style={{ flex: '0 0 auto', fontSize: 12 }}>{formatHours(shiftMinutes(shift))}</strong>
                                    </div>
                                    ...
                                    <div style={{ fontSize: 10, marginTop: 3, opacity: .72 }}>{formatPauseDetails(shift)}</div>
                                  </div>
                                ) : isOff ? (
          ...
          {activeView === 'skello' && (
            <section className="card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              {skelloMode === 'day' ? (
                <div className="table-wrap global-view-wrap mobile-horizontal-scroll" tabIndex="0">
                      ...
                      <div style={{ display: 'grid', gap: 12 }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                          <button type="button" className="secondary" onClick={() => {
                            ...
                            updateShift(employeeIndex, dayKey, 'breakStart', '');
                            updateShift(employeeIndex, dayKey, 'breakEnd', '');
                    ...
                    {shiftPresets.map((preset, index) => (
                      <div className="shift-preset-row" key={`${preset.name}-${index}`}>
                        ...
                        <div style={{ display: 'grid', gap: 8 }}>
                          <label>Début pause<input type="time" value={preset.breakStart || ''} onChange={(e) => updateShiftPreset(index, 'breakStart', e.target.value)} /></label>
                          <label>Fin pause<input type="time" value={preset.breakEnd || ''} onChange={(e) => updateShiftPreset(index, 'breakEnd', e.target.value)} /></label>
                        </div>
                        <button className="secondary danger-button" onClick={() => removeShiftPreset(index)}>Supprimer ce shift</button>
                      </div>
                    ))}
                  </div>
                  <p className="hint left">Les shifts sont triés par nom. En sélectionnant un shift dans le planning, le guichet, l'heure de début, l'heure de fin, la durée et les horaires de pause sont remplis automatiquement.</p>
                </div>
