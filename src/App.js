...
const defaultShiftPresets = [
  ...
  { name: 'Bureau 07h00', place: 'Bureau', start: '07:00', end: '15:30', breakMinutes: '0,50', breakStart: '13:00', breakEnd: '13:30' },
  { name: 'Bureau 07h30', place: 'Bureau', start: '07:30', end: '16:00', breakMinutes: '0,50', breakStart: '13:00', breakEnd: '13:30' },
...
function withPdfPauseTimes(item, label = '') {
  ...
  if (normalizedLabel.includes('max journée') && pauseMinutes === 90) {
    return { ...item, breakStart: '13:00', breakEnd: '14:30' };
  }
  if (normalizedLabel.includes('bureau') && pauseMinutes === 30) {
    return { ...item, breakStart: '13:00', breakEnd: '13:30' };
  }
  return item;
}
...
function migrateShiftPresetPauses(storedPresets) {
  ...
  return presets.map((preset) => {
    if (!preset?.breakStart && !preset?.breakEnd && String(preset?.name || '').toLocaleLowerCase('fr-FR').includes('max journée')) {
      return withPdfPauseTimes({ ...preset, breakMinutes: '1,50' }, preset.name);
    }
    return withPdfPauseTimes(preset, preset?.name);
  });
}
