  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .admin-sticky-week-card { overflow: visible !important; }
        .admin-week-sticky { position: sticky; top: 0; z-index: 30; margin: -4px -4px 10px; padding: 14px 12px; border-bottom: 1px solid #dbe3ec; border-radius: 10px 10px 0 0; background: rgba(255,255,255,.97); box-shadow: 0 5px 12px rgba(15,23,42,.08); backdrop-filter: blur(8px); }
          ...
          {activeView === 'week' && (
            <div className="month-weeks-list">
              {visibleHourSheetSegments.map((segmentDays) => {
                ...
                return (
                  <section className={`card month-week-card ${isAdmin ? 'admin-sticky-week-card' : ''}`} key={`${displayWeekKey}-${toDateInputValue(firstDate)}`}>
                    <div className={`section-title ${isAdmin ? 'admin-week-sticky' : ''}`}>
                      <div>
