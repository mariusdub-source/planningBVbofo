...
const COVERAGE_START_MINUTES = 7 * 60;
const COVERAGE_END_MINUTES = 23 * 60;
...
function getTimelineSegment(startValue, endValue, startBoundary = COVERAGE_START_MINUTES, endBoundary = COVERAGE_END_MINUTES) {
  const start = timeToMinutes(startValue);
  let end = timeToMinutes(endValue);
  if (start === null || end === null) return null;
  if (end <= start) end += 24 * 60;
  const segmentStart = Math.max(startBoundary, start);
  const segmentEnd = Math.min(endBoundary, end);
  if (segmentEnd <= segmentStart) return null;
  return { start, end, segmentStart, segmentEnd };
}
...
function App() {
  ...
  const employeeBarColor = (employeeIndex) => `hsl(${(employeeIndex * 47) % 360} 72% 68%)`;
  const coverageHours = useMemo(() => Array.from({ length: (COVERAGE_END_MINUTES - COVERAGE_START_MINUTES) / 60 + 1 }, (_, index) => COVERAGE_START_MINUTES + index * 60), []);
  const coverageByPlace = useMemo(() => {
    return places.filter(Boolean).map((place) => {
      const shifts = employees.map((employee, employeeIndex) => {
        const shift = getShift(data, weekKey, employeeIndex, selectedDayKey);
        if (!shift?.counter || shift.counter !== place || !shift.start || !shift.end) return null;
        const segment = getTimelineSegment(shift.start, shift.end);
        ...
        return {
          ...
          pause: shift.breakMinutes || '0',
          note: shift.note || '',
          ...segment
        };
      }).filter(Boolean).sort((a, b) => a.segmentStart - b.segmentStart);

      const gaps = [];
      let cursor = COVERAGE_START_MINUTES;
      shifts.forEach((shift) => {
        if (shift.segmentStart > cursor) {
          gaps.push({ start: cursor, end: shift.segmentStart });
        }
        cursor = Math.max(cursor, shift.segmentEnd);
      });
      if (cursor < COVERAGE_END_MINUTES) {
        gaps.push({ start: cursor, end: COVERAGE_END_MINUTES });
      }
  ...
  }, [places, employees, data, weekKey, selectedDayKey]);
  ...
  const renderTimeSelect = (employeeIndex, dayKey, field, value, updater = updateShift) => {
    const listId = `time-options-${field}-${employeeIndex}-${dayKey}`;
    const suggestedTimes = Array.from(new Set(
      Array.from({ length: 24 }, (_, index) => pad(index))
        .flatMap((hourOption) => minuteDecimals.filter(Boolean).map((minuteOption) => buildTime(hourOption, minuteOption)))
        .filter(Boolean)
    ));
  ...
  return (
    <div className="app-shell">
        ...
        <>
          <section className="toolbar card">
            ...
            <div className="toolbar-actions">
              <button className="secondary week-arrow-button" onClick={() => changeWeek(-1)} aria-label="Semaine précédente" title="Semaine précédente">← <span>Semaine précédente</span></button>
              <button className="secondary" onClick={() => setSelectedDate(toDateInputValue(new Date()))}>Aujourd'hui</button>
              <button className="secondary week-arrow-button" onClick={() => changeWeek(1)} aria-label="Semaine suivante" title="Semaine suivante"><span>Semaine suivante</span> →</button>
              <button className="secondary" onClick={() => setShowCoveragePanel(true)}>Graphique guichets</button>
      ...
      {showCoveragePanel && canUsePlanning && (
        <div className="coverage-overlay" onClick={() => setShowCoveragePanel(false)}>
          <aside className="coverage-drawer" role="dialog" aria-modal="true" aria-labelledby="coverage-title" onClick={(e) => e.stopPropagation()}>
            <div className="coverage-drawer-header">
              <div>
                <h2 id="coverage-title">Couverture des guichets</h2>
                <p>{selectedDayLabel} {displayDate(selectedDateObject)} · plage horaire 07:00 → 23:00</p>
              </div>
              <button type="button" className="secondary" onClick={() => setShowCoveragePanel(false)}>Fermer</button>
            </div>
            <div className="coverage-explain">Les barres colorées représentent les présences planifiées par salarié. Les créneaux indiqués en orange correspondent aux plages sans personnel affecté sur le guichet.</div>
            <div className="coverage-place-list">
              {coverageByPlace.map((placeItem) => (
                <section className="coverage-place-card" key={placeItem.place}>
                  <div className="coverage-place-head">
                    <div>
                      <h3>{placeItem.place}</h3>
                      <p>{placeItem.employeeCount ? `${placeItem.employeeCount} salarié(s) planifié(s)` : 'Aucun salarié planifié'}</p>
                    </div>
                    <span className={`coverage-gap-badge ${placeItem.gaps.length ? 'warning' : 'ok'}`}>
                      {placeItem.gaps.length ? `${placeItem.gaps.length} manque(s)` : 'Couverture continue'}
                    </span>
                  </div>
                  <div className="coverage-hours-scale">
                    {coverageHours.map((hourMinutes) => (
                      <span key={`${placeItem.place}-${hourMinutes}`}>{minutesToTimeLabel(hourMinutes)}</span>
                    ))}
                  </div>
                  <div className="coverage-timeline-grid">
                    {coverageHours.slice(0, -1).map((hourMinutes) => (
                      <span key={`${placeItem.place}-line-${hourMinutes}`} className="coverage-grid-line" style={{ left: `${((hourMinutes - COVERAGE_START_MINUTES) / (COVERAGE_END_MINUTES - COVERAGE_START_MINUTES)) * 100}%` }} />
                    ))}
                    {placeItem.shifts.length ? placeItem.shifts.map((shiftItem) => (
                      <div className="coverage-employee-row" key={`${placeItem.place}-${shiftItem.employeeIndex}-${shiftItem.start}-${shiftItem.end}`}>
                        <div className="coverage-employee-name">{shiftItem.employee}</div>
                        <div className="coverage-track">
                          <div
                            className="coverage-bar"
                            style={{
                              left: `${((shiftItem.segmentStart - COVERAGE_START_MINUTES) / (COVERAGE_END_MINUTES - COVERAGE_START_MINUTES)) * 100}%`,
                              width: `${((shiftItem.segmentEnd - shiftItem.segmentStart) / (COVERAGE_END_MINUTES - COVERAGE_START_MINUTES)) * 100}%`,
                              backgroundColor: employeeBarColor(shiftItem.employeeIndex)
                            }}
                          >
                            <strong>{shiftItem.start} – {shiftItem.end}</strong>
                            <small>{shiftItem.pause ? `Pause ${shiftItem.pause} h` : 'Sans pause'}</small>
                          </div>
                        </div>
                      </div>
                    )) : (
                      <div className="coverage-empty">Aucun shift renseigné sur ce guichet pour cette journée.</div>
                    )}
                  </div>
                  <div className="coverage-gaps">
                    <strong>Créneaux sans personnel :</strong>{' '}
                    {placeItem.gaps.length
                      ? placeItem.gaps.map((gap) => `${minutesToTimeLabel(gap.start)}–${minutesToTimeLabel(gap.end)}`).join(' · ')
                      : 'aucun sur la plage 07:00–23:00'}
                  </div>
                </section>
              ))}
            </div>
